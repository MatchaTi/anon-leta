  ...
  return (
    <div className={`p-6 h-[410px] flex flex-col justify-between w-full ${postCardStyles()} max-w-[350px] shadow-lg`}>
      <div className="w-full overflow-hidden">
        <Link href={`${link ? '/letter/' + link : '/'}`} className="block mb-4 w-full max-h-[190px] overflow-hidden text-3xl leading-relaxed hyphens-auto md:text-5xl">{description}</Link>
      ...
      <Link href={`${link ? '/letter/' + link : '/'}`} className="p-4 flex gap-6 items-center bg-stone-100 rounded-lg">
        ...
        <div className="overflow-hidden">
          <h3 className="font-bold text-ellipsis truncate md:text-2xl">{titleSong}</h3>
          <p>{artist}</p>
        </div>
      </Link>
    </div>
